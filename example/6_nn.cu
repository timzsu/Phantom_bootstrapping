...
std::filesystem::path data_path = std::filesystem::path(std::source_location::current().file_name()).parent_path() / "data";
...
void MM_test() {
  ...
  auto ckks_evaluator = std::make_shared<CKKSEvaluator>(context, public_key, secret_key, encoder, relin_keys, galois_keys, SCALE, galois_elts);
  ...
  std::vector<std::vector<double>> matrix_4096x768 = mme.read_matrix(data_path / "input/matrixmul_input_m_128_n_768_k_64_batch_128.txt", 4096, 768);
  std::vector<std::vector<double>> matrix_768x64 = mme.read_matrix(data_path / "input/matrix_input_n_768_k_64.txt", 768, 64);
  ...
  std::vector<std::vector<double>> matrix_4096x64 = mme.read_matrix(data_path / "calibration/matrix_output_m_128_k_64_batch_128.txt", 4096, 64);
...
void argmax_test() {
  ...
  // QuickMax: 17
  int main_mod_count = COEFF_MODULI[1][0];
  ...
  ifstream input_file(data_path / "input/argmax_input_8.txt");
  ...
  ifstream calibration_file(data_path / "calibration/argmax_calibration_8.txt");
...
void example_nn(int target) {
  ...
  params.set_coeff_modulus(CoeffModulus::Create(N, COEFF_MODULI[target]));

  auto context = std::make_shared<PhantomContext>(params);
  auto secret_key = std::make_shared<PhantomSecretKey>(*context);
  auto public_key = std::make_shared<PhantomPublicKey>(secret_key->gen_publickey(*context));
  auto relin_keys = std::make_shared<PhantomRelinKey>(secret_key->gen_relinkey(*context));
  ...
  */
  if (target == 4) {
    ...
    ifstream input_file(data_path / "input/gelu_input_32768.txt");
    while (input_file >> num) {
      input.push_back(num);
    }
    input_file.close();

    ifstream calibration_file(data_path / "calibration/gelu_calibration_32768.txt");
    while (calibration_file >> num) {
  ...
  */
  if (target == 3) {
    ...
    ifstream input_file(data_path / "input/layernorm_input_16_768.txt");
    while (input_file >> num) {
      input.push_back(num);
    }
    input_file.close();

    ifstream calibration_file(data_path / "calibration/layernorm_calibration_16_768.txt");
    while (calibration_file >> num) {
  ...
  */
  if (target == 2) {
    ...
    ifstream input_file(data_path / "input/softmax_input_128_128.txt");
    while (input_file >> num) {
      input.push_back(num);
    }
    input_file.close();

    ifstream calibration_file(data_path / "calibration/softmax_calibration_128_128.txt");
    while (calibration_file >> num) {
